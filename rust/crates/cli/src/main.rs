...
fn save_cpp_telemetry(rows: &[openTsiolkovsky_core::simulator::CsvCppRow], path: &str) -> std::result::Result<(), IoError> {
    ...
    for r in rows {
        w.write_record(&[
            ...
            r.lon_deg.to_string(),
            r.alt_m.to_string(),
            r.pos_eci.x.to_string(), r.pos_eci.y.to_string(), r.pos_eci.z.to_string(),
            r.vel_eci.x.to_string(), r.vel_eci.y.to_string(), r.vel_eci.z.to_string(),
            r.vel_ned.x.to_string(), r.vel_ned.y.to_string(), r.vel_ned.z.to_string(),
            r.acc_eci.x.to_string(), r.acc_eci.y.to_string(), r.acc_eci.z.to_string(),
            r.acc_body.x.to_string(), r.acc_body.y.to_string(), r.acc_body.z.to_string(),
            r.isp_s.to_string(), r.mach.to_string(), r.att_az_deg.to_string(), r.att_el_deg.to_string(), r.att_roll_deg.to_string(),
            r.aoa_alpha_deg.to_string(), r.aoa_beta_deg.to_string(), r.aoa_gamma_deg.to_string(),
            r.q_pa.to_string(),
            r.aero_body.x.to_string(), r.aero_body.y.to_string(), r.aero_body.z.to_string(),
            r.thrust_body.x.to_string(), r.thrust_body.y.to_string(), r.thrust_body.z.to_string(),
            r.gimbal_pitch_deg.to_string(), r.gimbal_yaw_deg.to_string(),
            r.wind_speed.to_string(), r.wind_dir_deg.to_string(), r.downrange.to_string(),
            r.iip_lat_deg.to_string(), r.iip_lon_deg.to_string(),
            r.dcm_body_to_eci[(0,0)].to_string(), r.dcm_body_to_eci[(0,1)].to_string(), r.dcm_body_to_eci[(0,2)].to_string(),
            r.dcm_body_to_eci[(1,0)].to_string(), r.dcm_body_to_eci[(1,1)].to_string(), r.dcm_body_to_eci[(1,2)].to_string(),
            r.dcm_body_to_eci[(2,0)].to_string(), r.dcm_body_to_eci[(2,1)].to_string(), r.dcm_body_to_eci[(2,2)].to_string(),
            r.inertial_speed.to_string(), r.kinetic_energy_ned.to_string(),
            r.loss_gravity.to_string(), r.loss_aero.to_string(), r.loss_thrust.to_string(),
            r.is_powered.to_string(), r.is_separated.to_string(),
        ])?;
    }
    w.flush()?;
    Ok(())
