...
pub fn save_trajectory_csv<P: AsRef<Path>>(trajectory: &[SimulationState], output_path: P) -> Result<()> {
...
pub fn save_summary_json<P: AsRef<Path>>(trajectory: &[SimulationState], config: &RocketConfig, output_path: P) -> Result<()> {
    ...
    let max_altitude_state = trajectory.iter().max_by(|a, b| a.altitude.partial_cmp(&b.altitude).unwrap()).unwrap();
    let max_velocity_state = trajectory.iter().max_by(|a, b| a.velocity_magnitude.partial_cmp(&b.velocity_magnitude).unwrap()).unwrap();
    let max_mach_state = trajectory.iter().max_by(|a, b| a.mach_number.partial_cmp(&b.mach_number).unwrap()).unwrap();
