...
impl GravityModel {
    ...
    pub fn gravity_magnitude(&self, altitude: f64, latitude_rad: f64) -> f64 {
        ...
        let mu = MU_EARTH;
        let gc = mu / (r * r) * (1.0 - 1.5 * j2 * (re / r).powi(2) * 
                                (3.0 * latitude_rad.sin().powi(2) - 1.0));
...
mod tests {
    ...
    fn test_point_mass_vs_wgs84() {
        ...
        let relative_error = (gravity_wgs84 - gravity_point_mass).magnitude() / gravity_point_mass.magnitude();
