use crate::{io::create_rocket_from_config, simulator::Simulator};

#[test]
fn run_sample_param_trajectory() {
    // Use sample JSON from repo examples/
    let param_path =
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("examples/param_sample_01.json");
    assert!(
        param_path.exists(),
